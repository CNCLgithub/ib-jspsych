...
const db = new sqlite3.Database('./conditions.db', (err) => {
  if (err) {
          ...
          if (err) {
            ...
            conditions.forEach((cond) => {
              db.run(
                ...
                [cond],
                (err) => {
                  if (err) console.error('Error initializing condition:', err.message);
                }
              );
            });
          }
    ...
    db.run(`
        ...
        `, (err) => {
          if (err) {
            console.error('Error creating pending_assignments table:', err.message);
          }
        });
  }
});
...
app.get('/candidate-condition', async (req, res) => {
  ...
  try {
    ...
    let eligibleConditions = rows.filter(row => row.count === minCount).map(row => row.condition);
    let candidateCondition = eligibleConditions[Math.floor(Math.random() * eligibleConditions.length)];
...
app.post('/confirm-condition', async (req, res) => {
  ...
  try {
    ...
    if (!pending || pending.condition !== condition) {
      return res.status(400).json({ error: 'Invalid or missing pending assignment' });
    }

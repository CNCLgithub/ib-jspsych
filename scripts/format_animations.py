...
def write_positions(trials_raw: dict, filename="dots.bin"):
    dataset = dots_pb2.Dataset()
    for trial_raw in trials_raw:
        trial = dataset.trials.add()

        if 'gorilla' in trial_raw:
            gorilla = trial.gorilla
            gorilla.frame = trial_raw['gorilla']['frame']
            gorilla.parent = trial_raw['gorilla']['parent']
            gorilla.speedx = trial_raw['gorilla']['speedx']
            gorilla.speedy = trial_raw['gorilla']['speedy']
    ...
    print(f"Wrote {len(trials_raw)} trials to {filename}")
...
def read_positions(filename="dots.bin"):
    ...
    print(f"Read {len(positions)} trials from {filename}")
    return positions
...
if __name__ == "__main__":
    ...
    write_positions(trials_raw, outpath)
